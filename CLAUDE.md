...
After both fixes, X-position and width land exactly on the cell boundary in every tested preset, confirmed on the physical 6-monitor wall — no more bleeding across monitors. A smaller, separate issue remains: Character Map and Paint both consistently clamp their own *height* to ~82% of whatever is requested (e.g. asked for 1080, settles at 881 — reproduced identically across single-cell and multi-cell layouts). Width is never clamped, only height.

Tried and reverted: re-asserting `SetWindowPos` on a delay after launch (in case the app was fighting our resize during its own startup layout). Made no difference — the ~82% ceiling held even after four repeated attempts up to 2s later, confirming this is a hard internal constraint (almost certainly `WM_GETMINMAXINFO`) rather than a timing race. Not pursued further; fixing it would need WndProc subclassing to intercept that message, which is invasive for arbitrary third-party apps. Both Character Map and Paint are fixed/dialog-style Windows utilities, not the kind of freely-resizable "standard windowed app" the architecture assumes — a real target dashboard app should not be expected to hit this.
