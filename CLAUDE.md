...
- **File Explorer:** a different, worse failure mode than Notepad's — `explorer.exe` hands the request to the already-running shell (which opens a real, uncontrolled window on the desktop) and then exits immediately, rather than hanging around to time out. **Fixed 2026-09-16:** the old retry logic treated this the same as a timeout and retried 5 times, meaning a failed File Explorer binding opened 5-6 real stray windows on the desktop — confirmed live when this happened during testing. Added `ProcessExitedWithoutWindowException` to distinguish "exited immediately, side effect likely already happened" from "timed out, probably still just stuck" — the former now gives up after the first attempt instead of retrying, since retrying only repeats the uncontrolled side effect.
