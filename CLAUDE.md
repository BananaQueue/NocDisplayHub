...
## Wall auto-orient + terminate control (2026-09-17)
Two operability gaps found through live use, both in `CompositorWindow`:

**"Launch Wall" always opened at (0,0), not necessarily the hub.** The XAML had `Left="0" Top="0"` hardcoded. On the real kiosk that's correct — the hub's display is the *only* display, so (0,0) is always right. But wherever more than one screen is actually attached to the Windows session (the dev machine, or the hub temporarily sharing a workstation with a normal monitor), (0,0) is just whichever screen happens to be primary, which is not necessarily the hub. Fixed with `HubDisplayLocator.FindHubOrigin()` — enumerates Windows' GDI screens via `EnumDisplayMonitors`/`GetMonitorInfo` and looks for the *one* screen whose resolution exactly matches `HubSpec` (1920×1080), positioning the compositor there. If zero or more than one screen matches, it can't tell which is the hub, so it deliberately falls back to (0,0) and writes an activity-log entry instead of guessing. This is a different thing from the multi-monitor rule elsewhere in this file: it does not treat the hub's 6 physical monitors as 6 OS monitors — it's a one-time "which single screen do I cover" lookup before positioning the one compositor window, run once at construction. Verified live on the dev machine, which has the hub attached as a second screen (reported by Windows as "MACROSILICON", the capture-chip name) alongside the built-in laptop display: the wall now opens on the hub screen automatically instead of covering the laptop's own display.

**No way to end the wall except Alt+F4 or Task Manager.** `WindowStyle="None"` means there's no title bar or close button, and nothing in the app had ever added an exit path. Added a small "■ STOP WALL" control, dim until hovered, in the top-right corner. It has to be a *separate* `Topmost` window (`TerminateOverlay`), not a WPF button drawn into `CellCanvas` — reparented native app windows and WebView2 both paint via their own child HWND, and a plain WPF element at the same screen position always loses that airspace fight regardless of its Z-order in the visual tree. `TerminateOverlay`'s `Owner` is set to the `CompositorWindow`, so WPF closes it automatically whenever the wall closes by any other means too. Verified live: the control stayed visible and clickable over live cell content, and clicking it closed the wall cleanly, leaving the editor (which never closes when the wall launches) running behind it.
