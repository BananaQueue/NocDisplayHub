...
- **Hub bezel compensation:** ~~Confirm whether the hub compensates for bezel gaps itself or whether the app needs to inset content per cell.~~ **Resolved 2026-09-16:** confirmed on real hardware — no bezel-gap inset needed, an even split lines up exactly across all five presets.
- **Hub per-port status feed:** ~~Confirm whether the hub exposes any way (serial, IP, HDMI CEC, etc.) to report an individual monitor's health, since Windows itself can't detect this through the composited signal.~~ **Decided 2026-09-16:** not pursuing this for v1. "Monitor went dark" detection stays at the cruder cell-level approach already built (process/render-crash detection via the watchdog) rather than a true physical-disconnect signal.
